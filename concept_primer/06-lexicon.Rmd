...
lambda parameter
: A parameter calibrated for the base year which is multiplied by the ratio of average freeway speed to average arterial speed to calculate the split of light-duty vehicle VMT between freeways and arterials in an urbanized area. The calibrated lambda parameter reflects the characteristics of the urbanized area road system and population. The calibrated lambda parameter is calculated from observed percentages using data from 90 urbanized areas. The calculation is carried out in the `CalculateRoadPerformance` module.
: [Additional details](https://github.com/visioneval/VisionEval/blob/master/api/VE_Training_March_13-14_2019/VE_Training.md#94-calculateroadperformance-module)
