...
- [**azone_carsvc_characteristics.csv**](#azone_carsvc_characteristics.csv-optional) This file specifies the different characteristics for high and low car service level and is used in the [CreateVehicleTable](Modules_and_Outputs.md/#createvehicletable) and [AssignVehicleAge](Modules_and_Outputs.md/#assignvehicleage) modules. 
...
User needs to prepare to run a VERSPM model are the following:

- Local PUMS data
- Check 5 definition files
- Prepare 51 input files

### azone_carsvc_characteristics.csv (optional) {-}
...
### azone_electricity_carbon_intensity.csv (optional) {-}

This input file specifies the arbon intensity of electricity by `Azone`. Using and changing this input is *optional* and only needed if the user wants to modify the carbon intensity of electricity.
...
This input file supplies data for retail cost of fuel and electricity by `Azone`. This input can be developed using local history or querying the Energy Information Administration (EIA) for historical [gasoline and diesel](https://www.eia.gov/petroleum/) and [power](https://www.eia.gov/electricity/) prices. 

- **FuelCost**: Average retail cost of fuel per gas gallon equivalent in dollars (before taxes are added)
- **PowerCost**: Average retail cost of electric power per kilowatt-hour in dollars (before taxes are added)
