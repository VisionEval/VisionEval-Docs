...
![RStudio Build GUI 1](images/buildgui-1.png)

4. Set the package directory to the VE module (e.g., src/VEGenericPackage) and the build source
![RStudio Build GUI 2](images/buildgui-2.png)

5. Build and install the package.
![RStudio Build GUI 3](images/buildgui-3.png)
...
Although the previous step generated the R data files, your current VisionEval environment will still be using the old package build data. You will need to re-build and re-install the VESimHousehold package for VisionEval to use this new input data. Any of the three build processes described above can be used.

Once complete, your new PUMS data should now be ready to use in a VisionEval model run.
...
This second case study explores a common challenge when certain inputs are embedded in a package and users want to modify the model values within the package. The critical purpose of the VEPowertrainsandFuels package is to process vehicle and fuel characteristics files that model users may optionally supply. Users can develop custom scenarios by modifying the relevant input files included in the 'inst/extdata' directory of the source package and build the package.
...
The previous step generated the R data files, and you will need to re-build and re-install the VESimHousehold package for VisionEval to use this new input data. Any of the three build processes described above can be used.

Once complete, your new powertrain data should now be ready to use in a VisionEval model run.
