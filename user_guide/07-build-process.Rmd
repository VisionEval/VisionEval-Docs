...
- Restimate or update the data in the package into a VisionEval datastore
...
**Note**: It is recommended to build VisionEval packages using a version of VisionEval built from the source code located on the [VisionEval github repository](https://github.com/VisionEval/VisionEval). The zipped VisionEval installer has been pre-built and bundled together with all necessary dependencies embedded, making it difficult to modify individual VisionEval packages without creating dependency issues.

### Generic VisionEval Module
VisionEval packages will generally have a structure similar to as follows:
```
...
└───inst
    └─ extdata
        ...
        └─ input_data2.txt
```
- `inst\extdata` is where any input data files will be placed
...
The package might function by first creating the datastore files (`.rda`) from the two comma separated value (`.csv`) files using the `CreateEstimationDatasets.R` script. When the package is called by VisionEval, a function in `GenericModel.R` runs a model using easily accessed data in the datastore.
