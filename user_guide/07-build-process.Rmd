...
## Introduction
The build process is a process for updating core VisionEval packages to incorporate 



The purpose of this chapter is to familiarize you with the build process for creating or modifying VisionEval packages. This chapter is intended to walk you through the process of substituting data and re-building a typical VE package, using the VESimHouseholds package as a case study in this example. 



**Note**: If you are going to build a package - Start with the Source version of VisionEval - find it here...[ link ]
...
- Restimate or update the data in the package
- Build the package for use in VE
...
+++Action: [ put a link to the description of the VESimHouseholds package. Describe - and link to the inputs and the outputs.   ]



The package functions by reading in two comma separated value (CSV) files with linked tables containing disaggregated persons and households data. These data come from Public Use Microdata Sample (PUMS) from the 2000 Decennial US Census. By default, the data in VisionEval is shipped using Oregon-based PUMS. 

The objective of this case study is to substitute the default Oregon-based data with another State PUMS data.


## Tutorial Objectives
1. Preprocess and format ``raw'' PUMS data into the comma separated value (CSV) files for VisionEval input
2. Create the estimated dataset stored as R data files (.Rda) for VisionEval (commonly referred to as "datastore format)   ++{do we link to a definition of datastore}
...
{ Nick - will add the ASC Scripts - for using more recent acs data  }
...
- we need to note where and how this proces may differ from other packages? for instance, can we say - this package includes an estimation script. but others do not.. ]
